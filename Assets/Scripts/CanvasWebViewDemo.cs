    ...
    class CanvasWebViewDemo : MonoBehaviour {
        ...
        async void Start() {
            ...
            canvasWebViewPrefab.WebView.PageLoadScripts.Add(
                @"const specialCodeValues = { '`': 'Backquote', '~': 'Backquote', '!': 'Digit1', '@': 'Digit2', '#': 'Digit3', '$': 'Digit4', '%': 'Digit5', '^': 'Digit6', '&': 'Digit7', '*': 'Digit8', '(': 'Digit9', ')': 'Digit0', '-': 'Minus', '_': 'Minus', '=': 'Equal', '+': 'Equal', '[': 'BracketLeft', '{': 'BracketLeft', ']': 'BracketRight', '}': 'BracketRight', '\\': 'Backslash', '|': 'Backslash', ';': 'Semicolon', ':': 'Semicolon', '\'': 'Quote', '""': 'Quote', 'Shift': 'ShiftLeft', ',': 'Comma', '<': 'Comma', '.': 'Period', '>': 'Period', '/': 'Slash', '?': 'Slash', 'Control': 'ControlLeft', 'Alt': 'AltLeft', 'Meta': 'MetaLeft', ' ': 'Space'};
                Object.defineProperty(KeyboardEvent.prototype, 'code', {
                    get: function() {
                        const key = this.key;
                        if (specialCodeValues[key]) {
                            return specialCodeValues[key];
                        }
                        if (key.length === 1) {
                            if (key >= '0' && key <= '9') {
                                return 'Digit' + key;
                        ...
                        // Used for keys like Escape, Tab, Enter, and CapsLock
                        return key;
                    }
                })");
        }
